# AIBP C&E Thailand 2026 — Post-Event Report

Standalone Next.js app serving the post-event report for the 55th AIBP
Conference & Exhibition Thailand 2026. Structured the same way as its
Indonesia sibling (`aibp-ce-post-id`), which was itself modeled on Malaysia's
(`aibp-ce-post-my`) — no live Sheets-backed operational app to borrow
Config/Sponsors from, so this repo's GAS backend is fully self-contained —
everything it serves lives on one report content spreadsheet.
...
Sheet ("CETH_Report_Content", `1S90-BGEp4dglA71g36wgImFJ-X-QuLVoxQI97A8ZotI`),
read live by `gas/Report.gs` on every `getReport` call (5-minute cache).
...
results even once a custom domain (`th.aibp.sg`) is attached.

## Colour scheme

Uses the Thailand secondary brand colour (`#834924`, "Thailand Brown") per
AIBP brand guidelines, with custom-derived dark/mid gradient shades — see
`app/ReportView.tsx` and `app/globals.css`. Everything else (typography,
layout, section structure) matches `aibp-ce-post-id`/`aibp-ce-post-my`
exactly.

## Known gaps to fill in

- `HERO_ENDORSEMENTS` in `app/ReportView.tsx` currently has only one
  confirmed endorsement (MDES) — add more as they're confirmed.
- Exact event venue is a best-guess default ("W Hotel Bangkok") in
  `gas/Report.gs` — confirm and add/override via a `Venue` row in
  `REPORT_META` if it changes.
- Photos, videos, sponsor logos, awards results, and testimonials are still
  being filled in on the content sheet.
